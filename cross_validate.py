...
group.add_argument('--ib-weight-cvlist', type=float, nargs=5, 
                   help='list of weight of IB loss for cv(default: 0.1)')
group.add_argument('--ib-warmup-ratio-cvlist', type=float, nargs=5, 
...
def run_cv(args, dataset_train_splits, device):
    ...
    for i in range(args.cvfold_number):
        ...
        #     torch.save(cluster_centroids.cpu(), '/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/clust_results/cluster_centroids_input.pt')
        ...
        feat_dim = 384
        class_num = 1000
        cluster_centroids_input = torch.load('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/clust_results/cluster_centroids_input.pt').to(device)
        ...
        try:
            for epoch in range(start_epoch, num_epochs):
                ...
                if dist.get_rank() == 0:
                    ...
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/clust_results/cluster_centroids_epoch_'+str(epoch)+'.npy', cluster_centroids.cpu().data.numpy())
                    ...
                    for batch_idx, (input, target) in enumerate(loader_train):
                        ...
                        with torch.no_grad():
                            ...
                            if dist.get_rank() == 0:
                                if batch_idx % 500 == 0:
                                    print(str(batch_idx)+ ' batches done in forward!')
                                np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/clust_results/out_feat_epoch_'+str(epoch)+'.npy', out_feat)
                                np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/clust_results/cluster_centroids_output_epoch_'+str(epoch)+'.npy', cluster_centroids.cpu().data.numpy())
                                # Compute p_in in the first epoch
                                clust_score_input = get_clust_score(input.view(input.shape[0], -1), cluster_centroids_input)
                                np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/clust_score_input_epoch_'+str(epoch)+'.npy', clust_score_input.cpu().data.numpy())
                                p_in = p_in + torch.sum(clust_score_input, dim=0)

                                clust_score_output = get_clust_score(feat, cluster_centroids.cuda(dist.get_rank()))
                                np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/clust_score_output_epoch_'+str(epoch)+'.npy', clust_score_output.cpu().data.numpy())
                                p_out = p_out + torch.sum(clust_score_output, dim=0)
                                ...
                                Q_numerator = Q_numerator + torch.matmul(clust_score_output.T, one_hot_target)
                                np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/Q_numerator_epoch_'+str(epoch)+'.npy', Q_numerator.cpu().data.numpy())
                                Q_denominator = Q_denominator + torch.sum(one_hot_target, dim=0)
                                np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/Q_denominator_epoch_'+str(epoch)+'.npy', Q_denominator.cpu().data.numpy())
                            dist.barrier()  # Make sure the task by rank 0 is done before others proceed
                if dist.get_rank() == 0:    
                    if epoch == start_epoch:
                        p_in = p_in / torch.sum(p_in)
                        np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/p_in_epoch_'+str(epoch)+'.npy', p_in.cpu().data.numpy())
                    p_out = p_out / torch.sum(p_out)
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/p_out_epoch_'+str(epoch)+'.npy', p_out.cpu().data.numpy())
                    p_label = p_label / torch.sum(p_label)
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/p_label_epoch_'+str(epoch)+'.npy', p_label.cpu().data.numpy())
                    p_in_out = p_in_out / torch.sum(p_in_out)
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/p_in_out_epoch_'+str(epoch)+'.npy', p_in_out.cpu().data.numpy())
                    p_out_label = p_out_label / torch.sum(p_out_label)
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/p_out_label_epoch_'+str(epoch)+'.npy', p_out_label.cpu().data.numpy())
                    ...
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/MI_in_out_log.npy', np.array(MI_in_out_log))
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/MI_out_label_log.npy', np.array(MI_out_label_log))
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/IB_log.npy', np.array(IB_log))

                    Q_raw = Q_numerator / Q_denominator
                    np.save('/home/local/ASUAD/ywan1053/ImageNet/pytorch-image-models/IB_logs/Q_raw_epoch_'+str(epoch)+'.npy', Q_raw.cpu().data.numpy())
    ...
    print('The average best metric is: ', best_metric_avg)
